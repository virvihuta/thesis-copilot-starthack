...
async def find_matches(query: SearchRequest):
    ...
    try:
        ...
        for doc in results:
            matches.append({
                # topic_id is what we use to look up the topic later (in generate-pitch)
                ...
                # snippet is the full embedded text — good for summaries
...
async def upload_cv(file: UploadFile = File(...)):
    ...
    try:
        ...
        results = db.similarity_search(text[:2000], k=3)  # limit to 2000 chars for token efficiency
